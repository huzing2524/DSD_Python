...
alter table base_clients_pool
    add column verify varchar(1) default '1';
-- verify  添加到资源池企业需经审核 0:未审核，1:已审核
...
alter table base_store_temporary_purchase
    add column reason varchar(60) default '';
-- 临时申购原因
...
alter table base_products
    ...
    add column lowest_product double precision default 0;
--最小生产量
...
-- 修改good和ng的数据类型
alter table base_product_task_processes
    alter column good type double precision,
    alter column ng type double precision;

-- 增加客户的送达时间
alter table base_clients
    add column deliver_days float default 0;
alter table base_suppliers
    add column deliver_days float default 0;
...
alter table base_purchases
    add column canceler varchar(36) references user_info (user_id);
alter table base_purchases
    add column cancel_remark varchar(60) default '';
alter table base_purchases
    add column arrival_time integer default 0;